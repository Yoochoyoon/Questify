  ...
  if (Math.random() < 0.5) {
    ...
    if (targetChar && !targetChar.showExclamation && !alreadyHasQuiz) {
      ...
      if (generated && generated.length > 0) {
        ...
        if (!newQuiz.content) { console.warn('[AI 퀴즈] content 필드 누락:', newQuiz); isGeneratingQuiz = false; return; }
        quizStorage.push({ id: uid(), title: newQuiz.title || "AI 생성 문제", content: newQuiz.content || "문제 내용", options: newQuiz.options || [], answer: newQuiz.answer || "자유 응답형", explanation: newQuiz.explanation || "해설이 제공되지 않았습니다.", type: quizType, charIdx: randomMemo.charIdx });
